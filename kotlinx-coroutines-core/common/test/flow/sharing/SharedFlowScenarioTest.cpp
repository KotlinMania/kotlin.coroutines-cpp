...
public:
    ...
    void test_replay1_extra2() {
        ...
        test_shared_flow(MutableSharedFlow<int>(1, 2), [this]() {
            // total buffer size == 3
            expect_replay_of();
            emit_right_now(1); expect_replay_of(1);
            emit_right_now(2); expect_replay_of(2);
            emit_right_now(3); expect_replay_of(3);
            emit_right_now(4); expect_replay_of(4); // no prob - no subscribers
            auto a = subscribe("a"); collect(a, 4);
            emit_right_now(5); expect_replay_of(5);
            emit_right_now(6); expect_replay_of(6);
            emit_right_now(7); expect_replay_of(7);
            // suspend/collect sequentially
            auto e8 = emit_suspends(8); collect(a, 5); emit_resumes(e8); expect_replay_of(8);
            auto e9 = emit_suspends(9); collect(a, 6); emit_resumes(e9); expect_replay_of(9);
            ...
            collect(a, 7); emit_resumes(e10); expect_replay_of(10); // buffer 8, 9 | 10
            collect(a, 8); emit_resumes(e11); expect_replay_of(11); // buffer 9, 10 | 11
            shared_flow.reset_replay_cache(); expect_replay_of(); // 9, 10 11 | no replay
            collect(a, 9); emit_resumes(e12); expect_replay_of(12);
            collect(a, 10, 11, 12); expect_replay_of(12); // buffer empty | 12
            emit_right_now(13); expect_replay_of(13);
            emit_right_now(14); expect_replay_of(14);
            emit_right_now(15); expect_replay_of(15); // buffer 13, 14 | 15
            auto e16 = emit_suspends(16);
            auto e17 = emit_suspends(17);
            auto e18 = emit_suspends(18);
            cancel(e17); expect_replay_of(15); // cancel in the middle of three emits; buffer 13, 14 | 15
            collect(a, 13); emit_resumes(e16); expect_replay_of(16); // buffer 14, 15, | 16
            collect(a, 14); emit_resumes(e18); expect_replay_of(18); // buffer 15, 16 | 18
            auto e19 = emit_suspends(19);
            auto e20 = emit_suspends(20);
            auto e21 = emit_suspends(21);
            cancel(e21); expect_replay_of(18); // cancel last emit; buffer 15, 16, 18
            collect(a, 15); emit_resumes(e19); expect_replay_of(19); // buffer 16, 18 | 19
            collect(a, 16); emit_resumes(e20); expect_replay_of(20); // buffer 18, 19 | 20
            collect(a, 18, 19, 20); expect_replay_of(20); // buffer empty | 20
            emit_right_now(22); expect_replay_of(22);
            emit_right_now(23); expect_replay_of(23);
            emit_right_now(24); expect_replay_of(24); // buffer 22, 23 | 24
            auto e25 = emit_suspends(25);
            auto e26 = emit_suspends(26);
            auto e27 = emit_suspends(27);
            cancel(e25); expect_replay_of(24); // cancel first emit, buffer 22, 23 | 24
            shared_flow.reset_replay_cache(); expect_replay_of(); // buffer 22, 23, 24 | no replay
            auto b = subscribe("b"); // new subscriber
            collect(a, 22); emit_resumes(e26); expect_replay_of(26); // buffer 23, 24 | 26
            collect(b, 26);
            collect(a, 23); emit_resumes(e27); expect_replay_of(27); // buffer 24, 26 | 27
            collect(a, 24, 26, 27); // buffer empty | 27
            emit_right_now(28); expect_replay_of(28);
            emit_right_now(29); expect_replay_of(29); // buffer 27, 28 | 29
            ...
            emit_resumes(e30); emit_resumes(e32); emit_resumes(e33); expect_replay_of(33); // buffer 30, 32 | 33
            auto c = subscribe("c"); collect(c, 33); // replays
            cancel(e34);
            collect(a, 30); emit_resumes(e35); expect_replay_of(35); // buffer 32, 33 | 35
            cancel(e37);
            cancel(a); emit_resumes(e36); emit_resumes(e38); expect_replay_of(38); // buffer 35, 36 | 38
            collect(c, 35); emit_resumes(e39); expect_replay_of(39); // buffer 36, 38 | 39
            collect(c, 36, 38, 39); expect_replay_of(39);
            cancel(c); expect_replay_of(39); // replay stays
        });
    }
    ...
    void test_replay1() {
        ...
        test_shared_flow(MutableSharedFlow<int>(1), [this]() {
            emit_right_now(0); expect_replay_of(0);
            emit_right_now(1); expect_replay_of(1);
            emit_right_now(2); expect_replay_of(2);
            shared_flow.reset_replay_cache(); expect_replay_of();
            shared_flow.reset_replay_cache(); expect_replay_of();
            emit_right_now(3); expect_replay_of(3);
            emit_right_now(4); expect_replay_of(4);
            auto a = subscribe("a"); collect(a, 4);
            emit_right_now(5); expect_replay_of(5); collect(a, 5);
            emit_right_now(6);
            shared_flow.reset_replay_cache(); expect_replay_of();
            shared_flow.reset_replay_cache(); expect_replay_of();
            ...
            collect(a, 6); emit_resumes(e7); expect_replay_of(7);
            shared_flow.reset_replay_cache(); expect_replay_of();
            shared_flow.reset_replay_cache(); expect_replay_of(); // buffer 7 | -- no replay, but still buffered
            auto b = subscribe("b");
            collect(a, 7); emit_resumes(e8); expect_replay_of(8);
            collect(b, 8); // buffer | 8 -- a is slow
            auto e10 = emit_suspends(10);
            auto e11 = emit_suspends(11);
            auto e12 = emit_suspends(12);
            cancel(e9);
            collect(a, 8); emit_resumes(e10); expect_replay_of(10);
            collect(a, 10); // now b's slow
            cancel(e11);
            collect(b, 10); emit_resumes(e12); expect_replay_of(12);
            collect(a, 12);
            collect(b, 12);
            shared_flow.reset_replay_cache(); expect_replay_of();
            shared_flow.reset_replay_cache(); expect_replay_of(); // nothing is buffered -- both collectors up to date
            emit_right_now(13); expect_replay_of(13);
            ...
            auto e16 = emit_suspends(16);
            cancel(e14);
            cancel(a); emit_resumes(e15); expect_replay_of(15); // cancelling slow subscriber
            collect(b, 15); emit_resumes(e16); expect_replay_of(16);
            collect(b, 16);
        });
    }
    ...
    void test_replay2_extra2_drop_oldest() {
        ...
        test_shared_flow<int>(MutableSharedFlow(2, 2, BufferOverflow::kDropOldest), [this]() {
            emit_right_now(0); expect_replay_of(0);
            emit_right_now(1); expect_replay_of(0, 1);
            emit_right_now(2); expect_replay_of(1, 2);
            emit_right_now(3); expect_replay_of(2, 3);
            emit_right_now(4); expect_replay_of(3, 4);
            auto a = subscribe("a");
            collect(a, 3);
            emit_right_now(5); expect_replay_of(4, 5);
            emit_right_now(6); expect_replay_of(5, 6);
            emit_right_now(7); expect_replay_of(6, 7); // buffer 4, 5 | 6, 7
            emit_right_now(8); expect_replay_of(7, 8); // buffer 5, 6 | 7, 8
            emit_right_now(9); expect_replay_of(8, 9); // buffer 6, 7 | 8, 9
            collect(a, 6, 7);
            auto b = subscribe("b");
            collect(b, 8, 9); // buffer | 8, 9
            emit_right_now(10); expect_replay_of(9, 10); // buffer 8 | 9, 10
            collect(a, 8, 9, 10); // buffer | 9, 10, note "b" had not collected 10 yet
            emit_right_now(11); expect_replay_of(10, 11); // buffer | 10, 11
            emit_right_now(12); expect_replay_of(11, 12); // buffer 10 | 11, 12
            emit_right_now(13); expect_replay_of(12, 13); // buffer 10, 11 | 12, 13
            emit_right_now(14); expect_replay_of(13, 14); // buffer 11, 12 | 13, 14, "b" missed 10
            collect(b, 11, 12, 13, 14);
            shared_flow.reset_replay_cache(); expect_replay_of(); // buffer 11, 12, 13, 14 |
            shared_flow.reset_replay_cache(); expect_replay_of();
            collect(a, 11, 12, 13, 14);
            emit_right_now(15); expect_replay_of(15);
            collect(a, 15);
            collect(b, 15);
        });
    }
    ...
    void test_resume_fast_subscriber_on_resumed_emitter() {
        ...
        test_shared_flow<int>(MutableSharedFlow(1), [this]() {
            ...
            auto s1 = subscribe("s1"); resume_collecting(s1);
            auto s2 = subscribe("s2"); resume_collecting(s2);
            ...
            emit_resumes(e2); expect_replay_of(2);
            // now emit 3, it suspends (s2 blocks it)
            auto e3 = emit_suspends(3);
            collect(s2, 2);
            emit_resumes(e3); expect_replay_of(3);
        });
    }
    ...
    private:
        ...
        TestJob launch_emit(T a) {
            ...
            Job* job = scope.launch(CoroutineStart::kUndispatched, [&]() {
                TestJob job_obj{/*coroutineContext[Job]!*/ nullptr, name};
                try {
                    log_msg(name);
                    shared_flow.emit(a);
