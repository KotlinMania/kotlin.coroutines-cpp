    ...
    void test_stress_notify() {
        runTest([&]() {
            ...
            for (int sender_id = 0; sender_id < n_senders; ++sender_id) {
                senders.push_back(launch(Dispatchers::Default + CoroutineName("Sender" + std::to_string(sender_id)), [&, sender_id]() {
                    // TODO: suspend function
            ...
            for (int receiver_id = 0; receiver_id < n_receivers; ++receiver_id) {
                receivers.push_back(launch(Dispatchers::Default + CoroutineName("Receiver" + std::to_string(receiver_id)), [&]() {
                    // TODO: suspend function
                    int last = -1;
                    while (is_active()) {
            ...
            auto progress_job = launch([&]() {
                ...
                while (true) {
                    delay(1000);
                    std::cout << (++seconds) << ": Sent " << sent_total.load() << ", received " << received_total.load() << std::endl;
                }
            });
            try {
            ...
            progress_job.cancel();
            std::cout << "Tested with nSenders=" << n_senders << ", nReceivers=" << n_receivers << std::endl;
            std::cout << "Completed successfully " << senders_completed.load() << " sender coroutines" << std::endl;
            std::cout << "Completed successfully " << receivers_completed.load() << " receiver coroutines" << std::endl;
