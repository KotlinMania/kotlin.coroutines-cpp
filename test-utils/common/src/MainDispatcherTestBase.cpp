...
public:
    ...
    virtual void spin_test(Job& test_body) { // TODO: implement coroutine suspension
        test_body.join();
    }
    ...
    TestResult run_test_or_skip(std::function<void(CoroutineScope&)> block) { // TODO: implement coroutine suspension
        // written as a block body to make the need to return TestResult explicit
        return run_test([&](CoroutineScope& scope) { // TODO: implement coroutine suspension
            if (should_skip_testing()) return;
            Job test_body = scope.launch(Dispatchers::Default, [&]() { // TODO: implement coroutine suspension
                block(scope);
            });
            spin_test(test_body);
        });
    ...
    void test_main_dispatcher_ordering_in_main_thread() {
        run_test_or_skip([&](CoroutineScope& scope) { // TODO: implement coroutine suspension
            scope.with_context(Dispatchers::Main, [&]() { // TODO: implement coroutine suspension
    ...
    void test_immediate_dispatcher_yield() {
        run_test_or_skip([&](CoroutineScope& scope) { // TODO: implement coroutine suspension
            scope.with_context(Dispatchers::Main, [&]() { // TODO: implement coroutine suspension
                ...
                scope.launch(Dispatchers::Main.immediate, [&]() { // TODO: implement coroutine suspension
                    expect(2);
                    yield();
                    expect(4);
                });
    ...
    void test_entering_immediate_from_main() {
        run_test_or_skip([&](CoroutineScope& scope) { // TODO: implement coroutine suspension
            scope.with_context(Dispatchers::Main, [&]() { // TODO: implement coroutine suspension
                expect(1);
                Job job = scope.launch([&]() { // TODO: implement coroutine suspension
                    expect(3);
                });
                scope.with_context(Dispatchers::Main.immediate, [&]() { // TODO: implement coroutine suspension
                    expect(2);
                });
                job.join();
            });
...
private:
    void check_dispatch_requirements() { // TODO: implement coroutine suspension
...
public:
    ...
    void test_launch_in_main_scope() {
        run_test_or_skip([&](CoroutineScope& scope) { // TODO: implement coroutine suspension
            bool executed = false;
            with_main_scope([&](CoroutineScope& main_scope) { // TODO: implement coroutine suspension
                main_scope.launch([&]() { // TODO: implement coroutine suspension
                    check_is_main_thread();
                    executed = true;
                }).join();
    ...
    void test_failure_in_main_scope() {
        run_test_or_skip([&](CoroutineScope& scope) { // TODO: implement coroutine suspension
            ...
            with_main_scope([&](CoroutineScope& main_scope) { // TODO: implement coroutine suspension
                ...
                main_scope.launch(handler, [&]() { // TODO: implement coroutine suspension
                    check_is_main_thread();
                    throw TestException();
                }).join();
            });
    ...
    void test_cancellation_in_main_scope() {
        run_test_or_skip([&](CoroutineScope& scope) { // TODO: implement coroutine suspension
            with_main_scope([&](CoroutineScope& main_scope) { // TODO: implement coroutine suspension
                main_scope.cancel();
                main_scope.launch(CoroutineStart::kAtomic, [&]() { // TODO: implement coroutine suspension
                    check_is_main_thread();
                    delay(LONG_MAX);
                }).join();
            });
...
private:
    template<typename R>
    R with_main_scope(std::function<R(CoroutineScope&)> block) { // TODO: implement coroutine suspension
    ...
    void test_main_dispatcher_ordering() { // TODO: implement coroutine suspension
        with_context(Dispatchers::Main.immediate, [&]() { // TODO: implement coroutine suspension
            expect(1);
            launch(Dispatchers::Main, [&]() { // TODO: implement coroutine suspension
                expect(2);
            });
            with_context(Dispatchers::Main, [&]() { // TODO: implement coroutine suspension
                finish(3);
            });
        });
    }
...
public:
    ...
    public:
        ...
        void test_with_timeout_context_delay_no_timeout() {
            run_test_or_skip([&](CoroutineScope& scope) { // TODO: implement coroutine suspension
                expect(1);
                scope.with_timeout(1000, [&]() { // TODO: implement coroutine suspension
                    scope.with_context(Dispatchers::Main, [&]() { // TODO: implement coroutine suspension
                        check_is_main_thread();
                        expect(2);
                        delay(100);
        ...
        void test_with_timeout_context_delay_timeout() {
            run_test_or_skip([&](CoroutineScope& scope) { // TODO: implement coroutine suspension
                expect(1);
                try {
                    scope.with_timeout(300, [&]() { // TODO: implement coroutine suspension
                        // A substitute for withContext(Dispatcher.Main) that is started even if the 300ms
                        // timeout happens fsater then dispatch
                        scope.launch(Dispatchers::Main, CoroutineStart::kAtomic, [&]() { // TODO: implement coroutine suspension
                            check_is_main_thread();
                            expect(2);
                            delay(1000);
                            expect_unreached();
        ...
        void test_with_context_timeout_delay_timeout() {
            run_test_or_skip([&](CoroutineScope& scope) { // TODO: implement coroutine suspension
                ...
                try {
                    scope.with_context(Dispatchers::Main, [&]() { // TODO: implement coroutine suspension
                        scope.with_timeout(100, [&]() { // TODO: implement coroutine suspension
                            check_is_main_thread();
                            expect(2);
                            delay(1000);
                            expect_unreached();
