...
coroutines
.
selects import kotlinx.coroutines.testing.*
...
class SelectRendezvousChannelTest : TestBase()  {
...

 {
    ...
    @
    Test
    fun testSelectReceiveClosed() = runTest

    (expected =
 { it is ClosedReceiveChannelException })
 {
        expect(1)
        val channel = Channel<String>(Channel.RENDEZVOUS)
        channel.close()
    ...
    @
    Test
    fun testSelectReceiveWaitClosed() = runTest
...

 {
    ...
    // only for debugging
    internal fun<R> SelectBuilder<R>
    .
    default (block : suspend()
    ->
    R
    )
    =
...
private
 suspend fun checkNotBroken(c: Channel<Int>) {
        coroutineScope{
            launch{
                c.send(42)
            }
