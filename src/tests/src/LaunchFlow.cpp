 ...
 */
     ...
     */
                    ...
                    void on_each(std::function<void(CoroutineScope &, T)> action) {
                        ...
                        if (!exception_handlers.empty()) {
                            throw std::runtime_error(
                                "onEach block should be registered before exceptionHandlers block");
                        }
                    ...
                    void catch_handler(std::function<void(CoroutineScope &, ExceptionT)> action) {
                        // TODO: implement coroutine suspension
                        if (on_each == nullptr) {
                        ...
                        if (finally_handler != nullptr) {
                            throw std::runtime_error(
                                "exceptionHandlers block should be registered before finally block");
                        }
                        // @Suppress("UNCHECKED_CAST")
                        exception_handlers[typeid(ExceptionT).name()] = reinterpret_cast<Handler<Throwable>>(action);
                    }

                    void finally(std::function<void(CoroutineScope &, Throwable *)> action) {
                        // TODO: implement coroutine suspension
                        if (finally_handler != nullptr) {
