...
class SharingStartedWhileSubscribedTest : TestBase()  {
    ...
    fun testEqualsAndHashcode() {
        ...
        for (i in params) {
            for (j in params) {
                map[SharingStarted.WhileSubscribed(i, j)] = i
                to j
            }
        }
        for (i in params) {
    ...
    fun testDurationParams() {
        ...
        assertEquals(SharingStarted.WhileSubscribed(replayExpirationMillis = 0),
        ...
        assertEquals(SharingStarted.WhileSubscribed(replayExpirationMillis = Long.MAX_VALUE),
