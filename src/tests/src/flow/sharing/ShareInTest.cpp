...
class ShareInTest : TestBase()  {
...

 {
    ...
    @
    Test
    fun testReplay0Lazy() = testReplayZeroOrOne
    ...
    @
    Test
    fun testReplay1Lazy() = testReplayZeroOrOne
...
private
    fun testReplayZeroOrOne(replay: Int) = runTest
...
private
    fun testUpstreamCompletedOrFailed(failed: Boolean) = runTest
...
private
    fun testWhileSubscribed(threshold: Int, started: SharingStarted) = runTest
...
private
    ...
    Companion
    .
    WhileSubscribedAtLeast (threshold : Int
    )
    =
    SharingStarted {
        subscriptionCount->
        ...
        {
            if (it >= threshold) SharingCommand.START
...
private
    ...
    private
        ...
        val started: Boolean get() = _started
        .
        value
        fun start() = check
        ...
        suspend fun awaitStart() = withTimeout

        (timeLimit){_started.first {it}}

        suspend fun awaitStop() = withTimeout

        (timeLimit){_started.first {!it}}
    }

private
 suspend fun FlowState
    .
    track (block : suspend()
    ->
    Unit
