 ...
 */
            ...
            public:
                ...
                void test_replay0_default_buffer() {
                    check_buffer(DEFAULT_BUFFER_SIZE, [](auto &flow, auto *scope) {
                        return flow.share_in(*scope, SharingStarted::eagerly());
                    });
                }
                ...
                void test_replay1_default_buffer() {
                    check_buffer(DEFAULT_BUFFER_SIZE, [](auto &flow, auto *scope) {
                        return flow.share_in(*scope, SharingStarted::eagerly(), 1);
                    });
                }

                // @Test // buffer is padded to default size as needed
                void test_replay10_default_buffer() {
                    check_buffer(std::max(10, DEFAULT_BUFFER_SIZE), [](auto &flow, auto *scope) {
                        return flow.share_in(*scope, SharingStarted::eagerly(), 10);
                    });
                }

                // @Test // buffer is padded to default size as needed
                void test_replay100_default_buffer() {
                    check_buffer(std::max(100, DEFAULT_BUFFER_SIZE), [](auto &flow, auto *scope) {
                        return flow.share_in(*scope, SharingStarted::eagerly(), 100);
                    });
                }
                ...
                void test_default_buffer_keeps_default() {
                    check_buffer(DEFAULT_BUFFER_SIZE, [](auto &flow, auto *scope) {
                        return flow.buffer().share_in(*scope, SharingStarted::eagerly());
                    });
                }
                ...
                void test_override_default_buffer0() {
                    check_buffer(0, [](auto &flow, auto *scope) {
                        return flow.buffer(0).share_in(*scope, SharingStarted::eagerly());
                    });
                }
                ...
                void test_override_default_buffer10() {
                    check_buffer(10, [](auto &flow, auto *scope) {
                        return flow.buffer(10).share_in(*scope, SharingStarted::eagerly());
                    });
                }
                ...
                void test_buffer_replay_sum() {
                    check_buffer(41, [](auto &flow, auto *scope) {
                        return flow.buffer(10).buffer(20).share_in(*scope, SharingStarted::eagerly(), 11);
                    });
                }
            };
        } // namespace flow
...
// 3. Implement SharingStarted enumeration with eagerly
