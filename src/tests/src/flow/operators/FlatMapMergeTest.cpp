...
namespace kotlinx {
    namespace coroutines {
        namespace flow {
            ...
            class FlatMapMergeTest : public FlatMapMergeBaseTest {
            public:
                Flow<int> flat_map(Flow<int> flow_var, auto mapper) override {
                    return flow_var.flat_map_merge(mapper);
                }
                ...
                // TODO: @Test
                void testAtomicStart() {
                    // TODO: runTest {
                    try {
                        coroutine_scope([&]() {
                            auto job = coroutine_context()[Job{}];
                            auto flow_var = flow([](auto &emit) {
                                    ...
                                    .flat_map_merge([](auto it) {
                                        expect(4);
                                        return flow_of(it).on_completion([&]() { expectUnreached(); });
                                    })
                                    .on_completion([&]() { expect(6); });
