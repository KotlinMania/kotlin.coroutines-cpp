    ...
    namespace coroutines {
        namespace flow {
            ...
            public:
                ...
                void testEmptyFlow() {
                    // TODO: runTest {
                    auto flow_var = combine_latest(empty_flow<std::string>(), empty_flow<int>(),
            ...
            public:
                Flow<int> combine_latest(Flow<int> flow1, Flow<int> flow2, auto transform) override {
                    return combine_transform(std::vector<Flow<int> >{flow1, flow2},
                                             [=](auto &emit, std::vector<std::any> args) {
                                                 emit(transform(std::any_cast<int>(args[0]),
