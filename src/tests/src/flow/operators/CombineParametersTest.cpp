        ...
        namespace flow {
            ...
            public:
                ...
                void testFiveParameters() {
                    // TODO: runTest {
                    auto flow_var = combine(flow_of("1"), flow_of(2), flow_of("3"), flow_of(static_cast<uint8_t>(4)),
                                            flow_of(nullptr),
                ...
                void testFiveParametersTransform() {
                    // TODO: runTest {
                    auto flow_var = combine_transform(flow_of("1"), flow_of(2), flow_of("3"),
                                                      flow_of(static_cast<uint8_t>(4)), flow_of(nullptr),
                ...
                void testNonMatchingTypes() {
                    ...
                    auto flow_var = combine(flow_of(1), flow_of("2"),
                                            [](std::vector<std::any> args) {
                                                return std::to_string(std::any_cast<int>(args[0])) + std::any_cast<
                                                           std::string>(args[1]);
                                            });
                ...
                void testSingleVarargTransform() {
                    // TODO: runTest {
                    auto list = combine_transform(flow_of(1, 2, 3), [](auto &emit, auto args) {
                        emit(args[0]);
                    }).to_list();
                    assertEquals(std::vector<int>{1, 2, 3}, list);
                    // TODO: }
                    ...
                    for (int flows_count = 2; flows_count <= 5; ++flows_count) {
                        for (int flow_size = 1; flow_size <= 5; ++flow_size) {
                            ...
                            assertEquals(expected, combined,
                                         "Count: " + std::to_string(flows_count) + ", size: " + std::to_string(
                                             flow_size));
                        }
                    }
                    // TODO: }
