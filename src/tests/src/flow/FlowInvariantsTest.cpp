        ...
        namespace flow {
            ...
            private:
                ...
                void run_parametrized_test(
                    ...
                    run_test([&]() -> /* suspend */ void {
                        auto r1 = run_catching([&]() {
                            test_body([](auto block) { return flow(block); });
                        }).exception_or_null();
                        check(r1, expected_exception);
                        reset();

                        auto r2 = run_catching([&]() {
                            test_body([](auto block) { return abstract_flow(block); });
                        }).exception_or_null();
                ...
                void check(std::exception_ptr exception,
                           std::optional</* KClass<out Throwable> */> expected_exception) {
            ...
            public:
                ...
                void test_with_context_contract() {
                    run_parametrized_test<int>(
                        /* IllegalStateException::class */ std::nullopt, [](auto flow_factory) -> /* suspend */ void {
                ...
                void test_merge_violation() {
                    run_parametrized_test<int>(std::nullopt, [](auto flow_factory) -> /* suspend */ void {
                        ...
                        assert_fails_with<IllegalStateException>([&]() {
                            merge(flow_instance, flow_instance).to_list();
                        });
                        assert_fails_with<IllegalStateException>([&]() {
                            tricky_merge(flow_instance, flow_instance).to_list();
                        });
                    });
                }
            ...
            public:
                ...
                void test_empty_coroutine_context_transform_while() {
                    run_test([]() -> /* suspend */ void {
                        empty_context_test([](Flow<int> &flow) -> Flow<int> {
                            return flow.transform_while(
                                [](int it, FlowCollector<int> &collector) -> /* suspend */ bool {
                                    expect(it);
                                    collector.emit(it + 1);
                                    return true;
                                });
