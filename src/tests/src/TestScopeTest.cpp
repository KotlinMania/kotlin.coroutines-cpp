...
namespace kotlinx {
     ...
     */
                ...
                TestResult test_background_work_time_control() {
                    return run_test([&]() {
                        ...
                        job.join();
                        advance_time_by(std::chrono::milliseconds(199));
                        // should work the same for the background tasks
                        assert(i == 2);
                        assert(j == 4);
     ...
     */
                ...
                TestResult test_unconfined_background_work_not_preventing_timeout() {
                    return test_result_map([](auto it) {
                                               ...
                                               return run_test(UnconfinedTestDispatcher(),
                        ...
                        [&](auto it) {
                            ...
                            return run_test([&]() {
                                ...
                                if (auto *uncaught = dynamic_cast<UncaughtExceptionsBeforeTest *>(exception)) {
                                    ...
                                    fail(std::string("unexpected exception: ") + (
                                             exception ? exception->what() : "null"));
                                }
                            });
                        }
                    );
     ...
     */
            ...
            std::vector<CoroutineContext> TestScopeTest::invalid_contexts = {
                ...
                CoroutineExceptionHandler([](auto, auto) {
                }), // exception handlers can't be overridden
