 ...
 */
        ...
        CoroutineContext new_coroutine_context(CoroutineContext base_context, CoroutineContext added_context);
        ...
        template<typename T>
        T with_coroutine_context(CoroutineContext context, void *count_or_element, std::function<T()> block);

        // Declaration only

        // TODO: expect inline fun - platform-specific inline function
        template<typename T>
        T with_continuation_context(Continuation<void> *continuation, void *count_or_element, std::function<T()> block);
